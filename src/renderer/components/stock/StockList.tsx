...
interface StockRowProps {
  ...
  onToggleMenu: (stockId: string) => void
  onDelete: (id: string) => void
  onEdit: (stock: Stock) => void
  onMove: (stockId: string, groupId: string) => void
  onShowChart?: (symbol: string, name: string) => void
...
export const StockList: React.FC<StockListProps> = ({
  ...
  const handleToggleMenu = React.useCallback((stockId: string) => {
    setShowMenuId((prev) => (prev === stockId ? null : stockId))
  }, [])
      ...
      <div className="divide-y divide-gray-200/50 dark:divide-gray-700/50">
        {stocks.map((stock) => (
          <StockRow
            ...
            flashColor={flashColors[stock.id]}
